        ...
        <div className="flex items-center gap-3 mb-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="flex items-center gap-1.5 px-3 py-[6px] bg-[#ff7a59] hover:bg-[#ff5c35] text-white rounded-[4px] text-[13px] font-medium transition-colors"
            >
              Push all to HubSpot
            </button>
          </div>
      ...
      <div className="flex-1 flex flex-col bg-white overflow-hidden">
        {loading ? (
          ...
          <div className="text-center py-16">
            <Building2 className="w-14 h-14 text-[#7c98b6] mx-auto mb-4" />
            <p className="text-[#516f90] mb-4">No companies yet</p>
            <button
              onClick={() => setAddModalOpen(true)}
          ...
          <>
                ...
                <thead>
                  <tr className="bg-[#f5f8fa] border-y border-[#cbd6e2]">
                    <th className="w-[44px] px-3 py-2.5">
                      <input type="checkbox" checked={allPageSelected} onChange={toggleSelectAll} className="w-[16px] h-[16px] rounded border-[#cbd6e2] accent-[#ff7a59]" />
                    </th>
                    <th className="text-left px-3 py-2.5 text-[12px] font-semibold text-[#33475b] tracking-wide">Company name</th>
                    <th className="text-left px-3 py-2.5 text-[12px] font-semibold text-[#33475b] tracking-wide">Domain</th>
                    <th className="text-left px-3 py-2.5 text-[12px] font-semibold text-[#33475b] tracking-wide">Industry</th>
                    <th className="text-left px-3 py-2.5 text-[12px] font-semibold text-[#33475b] tracking-wide">Intent Score</th>
                    <th className="text-left px-3 py-2.5 text-[12px] font-semibold text-[#33475b] tracking-wide">Stage</th>
                    <th className="text-left px-3 py-2.5 text-[12px] font-semibold text-[#33475b] tracking-wide">Trend</th>
                    <th className="text-left px-3 py-2.5 text-[12px] font-semibold text-[#33475b] tracking-wide">Visits</th>
                    <th className="text-left px-3 py-2.5 text-[12px] font-semibold text-[#33475b] tracking-wide">Runs</th>
                    <th className="w-[44px]" />
                  </tr>
                </thead>
                <tbody>
                  {pagedAccounts.map((account) => (
                    ...
                    >
                      <td className="px-3 py-2.5" onClick={(e) => e.stopPropagation()}>
                        <input type="checkbox" checked={selected.has(account.account_id)} onChange={() => toggleSelect(account.account_id)} className="w-[16px] h-[16px] rounded border-[#cbd6e2] accent-[#ff7a59]" />
                      </td>
                      ...
                      <td className="px-3 py-2.5 text-[14px] text-[#33475b]">{account.domain || "—"}</td>
                      <td className="px-3 py-2.5 text-[14px] text-[#33475b]">{account.industry || "—"}</td>
                      <td className="px-3 py-2.5">
                        <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium ${
                          (account.latest_intent_score || 0) >= 7
                            ...
                            : "bg-gray-100 text-[#516f90]"
                        }`}>
                          {account.latest_intent_score?.toFixed(1) || "—"}
                        </span>
                      </td>
                      <td className="px-3 py-2.5 text-[14px] text-[#33475b]">{account.latest_intent_stage || "—"}</td>
                      <td className="px-3 py-2.5">
                      ...
                      <td className="px-3 py-2.5 text-[14px] text-[#33475b]">{account.visit_count_total}</td>
                      <td className="px-3 py-2.5 text-[14px] text-[#33475b]">{account.run_count}</td>
                      <td className="px-3 py-2.5" onClick={(e) => e.stopPropagation()}>
                        <button
                          onClick={async () => {
                          ...
                          className="text-xs text-[#516f90] hover:text-[#ff7a59] px-1.5 py-0.5 rounded border border-transparent hover:border-[#cbd6e2] transition-colors"
                        >
                          HS
                        </button>
                      </td>
            ...
            <div className="flex items-center justify-center gap-2 py-4 border-t border-[#cbd6e2]">
              <button onClick={() => setPage(p => Math.max(1, p - 1))} disabled={page === 1} className="flex items-center gap-1 text-[14px] text-[#516f90] hover:text-[#33475b] disabled:opacity-40 transition-colors">Prev</button>
              {Array.from({length: totalPages}, (_, i) => i + 1).map(p => (
                <button key={p} onClick={() => setPage(p)} className={`w-[28px] h-[28px] flex items-center justify-center border rounded-[4px] text-[14px] font-medium transition-colors ${p === page ? "border-[#ff7a59] text-[#ff7a59] bg-white" : "border-[#cbd6e2] text-[#33475b] bg-white hover:bg-[#f5f8fa]"}`}>{p}</button>
              ))}
              <button onClick={() => setPage(p => Math.min(totalPages, p + 1))} disabled={page === totalPages} className="flex items-center gap-1 text-[14px] text-[#516f90] hover:text-[#33475b] disabled:opacity-40 transition-colors">Next</button>
              <span className="ml-4 text-[13px] text-[#516f90]">{filteredAccounts.length} total · {PAGE_SIZE} per page</span>
            </div>
          </>
        )}
      </div>
    ...
    <>
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
        <div className="bg-white rounded-lg shadow-xl max-w-md w-full p-6" onClick={(e) => e.stopPropagation()}>
          <h2 className="text-lg font-semibold text-[#33475b] mb-4">Add Company</h2>
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              <label className="block text-sm font-medium text-[#33475b] mb-1.5">Company Name *</label>
              <input
                type="text"
                required
            ...
            <div>
              <label className="block text-sm font-medium text-[#33475b] mb-1.5">Domain (optional)</label>
              <input
                type="text"
                className="w-full border border-[#cbd6e2] rounded-[4px] px-3 py-2 text-[#33475b] focus:border-[#ff7a59] focus:ring-1 focus:ring-[#ff7a59]/30"
                value={domain}
            ...
            <div className="flex gap-2 pt-4">
              <button
                ...
                onClick={onClose}
                disabled={loading}
                className="flex-1 px-4 py-2 border border-[#cbd6e2] text-[#33475b] rounded-[4px] hover:bg-[#f5f8fa] disabled:opacity-50"
              >
                Cancel
              </button>
              <button

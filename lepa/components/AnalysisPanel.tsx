...
function CopyButton({ text }: { text: string }) {
  ...
  return (
    <button onClick={copy} className="flex items-center gap-1 px-2 py-1 text-xs text-[#767676] hover:text-[#484848] border border-[#DDDDDD] rounded hover:bg-[#F7F7F7]">
...
function Section({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div className="border border-[#DDDDDD] rounded-[4px] overflow-hidden">
      <div className="px-3 py-1.5 bg-[#eaf0f6] border-b border-[#DDDDDD]">
        <span className="text-[11px] font-semibold text-[#767676] uppercase tracking-wide">{label}</span>
      </div>
  ...
  return (
    <ResponsiveContainer width="100%" height={200}>
      <RadarChart data={chartData}>
        ...
        <PolarAngleAxis dataKey="subject" tick={{ fontSize: 11, fill: "#767676" }} />
    ...
    <div className="space-y-3 mt-2">
      {!!data.account_overview && (
        <Section label="Account Overview">
          <p className="text-sm text-[#484848]">{String(data.account_overview)}</p>
        </Section>
      )}
              ...
              <div key={i} className="flex items-start justify-between gap-2 py-1.5 border-b border-[#eaf0f6] last:border-0">
                <div className="min-w-0">
                  <div className="flex items-center gap-2">
                    <span className="text-sm font-medium text-[#484848]">{m.name}</span>
                  ...
                  <p className="text-xs text-[#767676]">{m.title}</p>
      ...
      {angles.length > 0 ? (
        <Section label="Outreach Angles">
          <ol className="space-y-1 list-decimal list-inside">
            {angles.map((a, i) => <li key={i} className="text-sm text-[#484848]">{a}</li>)}
          </ol>
        </Section>
      ) : null}
      ...
      {!!email?.body && (
        <Section label="Cold Email">
          <div className="flex items-start justify-between gap-2 mb-2">
            <p className="text-xs text-[#767676]"><span className="font-medium">Subject:</span> {email.subject}</p>
            <CopyButton text={`Subject: ${email.subject}\n\n${email.body}`} />
          </div>
          <p className="text-sm text-[#484848] whitespace-pre-line">{email.body}</p>
        </Section>
      )}
      ...
      {!!data.linkedin_message && (
        <Section label="LinkedIn Message">
          <div className="flex items-start justify-between gap-2">
            <p className="text-sm text-[#484848] whitespace-pre-line flex-1">{String(data.linkedin_message)}</p>
      ...
      {!!data.next_action && (
        <Section label="Recommended Next Action">
          <p className="text-sm text-[#484848]">{String(data.next_action)}</p>
        </Section>
      )}
    </div>
  );
...
export default function AnalysisPanel({
  ...
  return (
    <>
      ...
      {/* Panel */}
      <aside
        className="fixed top-0 right-0 h-full w-full max-w-xl bg-white border-l border-[#DDDDDD] shadow-xl z-50 flex flex-col"
        role="dialog"
        aria-label="Analysis results"
      >
        {/* Header */}
        <div className="flex items-center justify-between px-6 py-4 border-b border-[#DDDDDD] shrink-0">
          <h2 className="text-lg font-semibold text-[#484848]">Account Analysis</h2>
          <div className="flex items-center gap-2">
            {onPushHubspot && result && (
              <button
                onClick={onPushHubspot}
                className="flex items-center gap-1.5 px-3 py-1.5 bg-[#FF5A5F] hover:bg-[#e0504a] text-white text-xs font-medium rounded-[4px] transition-colors"
              >
            ...
            <button
              onClick={onClose}
              className="p-2 text-[#767676] hover:text-[#484848] hover:bg-[#F7F7F7] rounded-[4px] transition-colors"
        ...
        <div className="flex-1 overflow-y-auto p-6">
          {loading && (
            <div className="flex flex-col items-center justify-center py-16">
              <Loader2 className="w-12 h-12 text-[#FF5A5F] animate-spin mb-4" />
              <p className="text-base font-medium text-[#484848] mb-2">Analyzing account...</p>
              <p className="text-sm text-[#767676]">Researching company, tech stack, signals & leadership...</p>
            </div>
          )}
            ...
            <div className="space-y-6">
              ...
              <div className="pb-4 border-b border-[#DDDDDD]">
                <div className="flex items-start justify-between gap-4 mb-3">
                  <div>
                    <h3 className="text-xl font-bold text-[#484848]">{result.account_name || "Unknown Account"}</h3>
                    {result.domain && (
                      <a
                ...
                <div className="grid grid-cols-2 gap-3 text-sm">
                  {[
                    ...
                    <div key={i}>
                  <p className="text-xs font-medium text-[#767676] uppercase mb-0.5">{field.label}</p>
                  <p className="font-medium text-[#484848]">{field.value || "—"}</p>
                    </div>
                  ))}
                </div>
                {result.business_description && (
                  <p className="mt-3 pt-3 border-t border-[#DDDDDD] text-sm text-[#767676]">{result.business_description}</p>
                )}
              </div>

              {/* Persona & Intent */}
              <div className="grid grid-cols-2 gap-4">
                <div className="p-4 bg-[#F7F7F7] border border-[#DDDDDD] rounded-[4px]">
                  <h4 className="text-xs font-medium text-[#767676] uppercase mb-2">Visitor Persona</h4>
                  <p className="text-lg font-bold text-[#484848] mb-1">{result.persona?.label ?? "—"}</p>
                  <div className="h-1.5 bg-[#DDDDDD] rounded-full overflow-hidden">
                    <div className="h-full bg-[#FF5A5F] rounded-full" style={{ width: `${(result.persona?.confidence ?? 0) * 100}%` }} />
                  </div>
                  <p className="text-xs text-[#767676] mt-1">{Math.round((result.persona?.confidence ?? 0) * 100)}% confidence</p>
                </div>
                <div className="p-4 bg-[#F7F7F7] border border-[#DDDDDD] rounded-[4px]">
                  <h4 className="text-xs font-medium text-[#767676] uppercase mb-2">Buying Intent</h4>
                  <p className="text-2xl font-bold text-[#FF5A5F]">{result.intent?.score?.toFixed(1) ?? "—"}<span className="text-base font-normal text-[#767676]">/10</span></p>
              ...
              {/* AI Summary */}
              <div>
                <h4 className="text-xs font-medium text-[#767676] uppercase mb-2">AI Summary</h4>
                <p className="text-sm leading-relaxed text-[#484848]">{result.ai_summary}</p>
              </div>

              {/* Recommended Actions */}
              {result.recommended_sales_action && (
              <div>
                <h4 className="text-xs font-medium text-[#767676] uppercase mb-3">Recommended Actions</h4>
                <ol className="space-y-2 mb-3">
                  {(result.recommended_sales_action.actions ?? []).map((action, i) => (
                    <li key={i} className="flex items-start gap-2 text-sm text-[#484848]">
                      <span className="flex-shrink-0 w-5 h-5 bg-[#FF5A5F] text-white font-bold flex items-center justify-center rounded text-xs">
                        {i + 1}
                      </span>
                      {action}
                    </li>
                ...
                <div className="p-3 bg-[#F7F7F7] border border-[#DDDDDD] rounded-[4px]">
                  ...
                  <p className="text-sm text-[#484848]">{result.recommended_sales_action.outreach_angle}</p>
                </div>
                )}
              </div>
              )}
              ...
              {result.key_signals_observed?.length > 0 && (
                <div>
                  <h4 className="text-xs font-medium text-[#767676] uppercase mb-2">Key Signals</h4>
                  <div className="flex flex-wrap gap-2">
                    {result.key_signals_observed?.map((signal, i) => (
                      <span key={i} className="px-2.5 py-1 bg-[#FF5A5F]/10 text-[#FF5A5F] border border-[#FF5A5F]/20 rounded-[4px] text-xs font-medium">
                        {signal}
                      </span>
                    ))}
                  </div>
              ...
              {result.technology_stack && result.technology_stack.length > 0 && (
                <div>
                  <h4 className="text-xs font-medium text-[#767676] uppercase mb-2">Technology Stack</h4>
                  <div className="grid grid-cols-2 gap-2">
                    {result.technology_stack.slice(0, 6).map((tech, i) => (
                      <div key={i} className="p-2 bg-[#F7F7F7] border border-[#DDDDDD] rounded-[4px]">
                        <p className="text-sm font-medium text-[#484848]">{tech.name}</p>
                        <p className="text-xs text-[#767676] capitalize">{tech.category.replace("_", " ")}</p>
                      </div>
                    ))}
                  </div>
                </div>
              ...
              {result.icp_fit && (
                <div>
                  <h4 className="text-xs font-medium text-[#767676] uppercase mb-2">ICP Fit</h4>
                  <div className="flex items-center gap-2 mb-2">
                    <span className={`px-2.5 py-1 rounded-md text-sm font-bold ${
                      result.icp_fit?.overall_score >= 80 ? "bg-emerald-600 text-white" :
                      result.icp_fit?.overall_score >= 60 ? "bg-[#FF5A5F] text-white" :
                      "bg-amber-600 text-white"
                    }`}>
                      {result.icp_fit?.overall_score}/100
                    </span>
                    <span className="text-sm text-[#767676]">{result.icp_fit.tier}</span>
                  </div>
                </div>
              )}
              ...
              {result.buying_committee && result.buying_committee.length > 0 && (
                <div>
                  <h4 className="text-xs font-medium text-[#767676] uppercase mb-3">Buying Committee</h4>
                  <div className="space-y-2">
                    {result.buying_committee.map((member, i) => (
                      <div key={i} className="flex items-center justify-between p-3 bg-[#F7F7F7] border border-[#DDDDDD] rounded-[4px]">
                        <div className="flex items-center gap-3 min-w-0">
                          <div className="w-8 h-8 bg-[#DDDDDD] rounded-full flex items-center justify-center text-[#484848] font-bold text-xs shrink-0">
                            {member.name.charAt(0)}
                          </div>
                          <div className="min-w-0">
                            <div className="flex items-center gap-2">
                              <p className="text-sm font-medium text-[#484848] truncate">{member.name}</p>
                              {member.source_url && (
                            ...
                            <p className="text-xs text-[#767676] truncate">{member.title}</p>
                          </div>
                        </div>
                        <span className={`px-2 py-0.5 rounded-[4px] text-xs font-bold shrink-0 ${roleColors[member.role] || "bg-[#767676] text-white"}`}>
                          {member.role}
                        </span>
                      </div>
                    ))}
              ...
              {onDeepResearch && (
                <div className="pt-4 border-t border-[#DDDDDD]">
                  <div className="flex items-center justify-between mb-3">
                    <h4 className="text-xs font-medium text-[#767676] uppercase">Deep Research</h4>
                    <button
              ...
              <p className="text-xs text-[#767676]">Confidence: {Math.round(result.overall_confidence * 100)}%</p>
            </div>
          )}
        </div>
      </aside>
